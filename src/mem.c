...
static struct region alloc_region  ( void const * addr, size_t query ) {
    size_t demanded_size = region_actual_size((size_from_capacity((block_capacity) {query})).bytes);
    void* new_addr = map_pages(addr, demanded_size, MAP_FIXED_NOREPLACE );
    if (new_addr == MAP_FAILED)
        new_addr = map_pages(addr, demanded_size, 0);
    if (new_addr == MAP_FAILED)
    ...
    struct region new_region = {
            .addr = new_addr,
            .size = demanded_size,
            .extends = (addr == new_addr)
    };

    block_init(new_addr, (block_size){demanded_size}, NULL);
    return new_region;
}
...
void heap_term( ) {
    ...
    while (current_block){
        ...
        void *current_region_start = current_block;
        while (current_block->next && current_block->next == block_after(current_block)){
...
static bool split_if_too_big( struct block_header* block, size_t query ) {
    if (!block_splittable(block, query))
        return false;
    struct block_header* new_block_header = (struct block_header*) (block->contents + query);
    block_init(new_block_header, (block_size){block->capacity.bytes - query}, block->next);
    block->capacity = (block_capacity){query};
    block->next = new_block_header;
    return true;
}
